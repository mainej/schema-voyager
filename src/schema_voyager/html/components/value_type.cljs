...
(defmulti p
  (fn [{:keys [db/cardinality] :as attribute}]
    (into [cardinality] (dispatch attribute))))
...
(defn- heterogeneous-one-span [{:keys [db/tupleTypes]}]
  [:span "This attribute has " one-abbr " value, a " heterogeneous-abbr " " tuple-abbr " of length "
   (count tupleTypes) ". The tuple is of type "])
(defn- heterogeneous-many-span [{:keys [db/tupleTypes]}]
  [:span "This attribute has " many-abbr " values, " heterogeneous-abbr " " tuples-abbr " of length "
...
(defmethod p [:db.cardinality/one :db.type.tuple/heterogeneous ::default] [attribute]
  [:p
   ...
   [heterogeneous-types-list attribute]])
...
(defmethod p [:db.cardinality/one :db.type/ref :referred] [{:keys [db.schema/references]}]
  [:p "This attribute has " one-abbr " value, a " reference-abbr " to one " [util/coll-links references] "."])
(defmethod p [:db.cardinality/one :db.type/ref ::default] [_attribute]
  [:p "This attribute " references-abbr " " one-abbr " value."])
(defmethod p [:db.cardinality/many :db.type/ref :referred] [{:keys [db.schema/references]}]
  [:p "This attribute has " many-abbr " values, each a " reference-abbr " to one " [util/coll-links references] "."])
(defmethod p [:db.cardinality/many :db.type/ref ::default] [_attribute]
  [:p "This attribute " references-abbr " " many-abbr " values."])

;; scalars
(defmethod p [:db.cardinality/one ::default ::default] [{:keys [db/valueType]}]
  [:p "This attribute has " one-abbr " " [keyword-abbr valueType] " value."])
(defmethod p [:db.cardinality/many ::default ::default] [{:keys [db/valueType]}]
  [:p "This attribute has " many-abbr " " [keyword-abbr valueType] " values."])

(defmulti shorthand-span dispatch)
