...
** TODO Unify CLI to a single invocation
*** DONE many sources
...
*** output to ... anywhere else?
** TODO follow guidance in https://clojure.org/reference/deps_and_cli#_best_practices_for_tool_authors
** TODO Release on clojars
** TODO simplify spread of files
   * get rid of dev/ directory?
   * move assets/ into resources/
   * output dynamic, and standalone html to target directory
   * can resource/inline use directory besides resources?
     Looks like no, so can't put dataascript db in target. Is there somewhere better to put it?
